...
type Modal = { type: "pillar" } | { type: "topic" } | { type: "assign"; pillarId: string };
...
export default function AddNodeToolbar({
  ...
  async function handleCreatePillar() {
    ...
    setSaving(true);
    try {
      await createPillar({ niche: pNiche.trim(), name: pName.trim() });
      setModal(null);
      onRefresh();
    } finally {
      setSaving(false);
    ...
    try {
      await Promise.all(
        Array.from(selectedArticles).map((id) =>
          adminUpdateArticle(id, { pillar_id: modal.pillarId })
        )
      );
      setModal(null);
  ...
  return (
    <>
      <div className="flex items-center gap-2">
        <button
          onClick={openPillar}
          className="flex items-center gap-1.5 rounded-lg bg-[#f5a700] px-3 py-1.5 text-xs font-semibold text-white transition hover:bg-[#f5a700]/90"
        >
          <Plus size={12} /> Pillar
        </button>
        <button
          onClick={openTopic}
          className="border-[#242423]/12 flex items-center gap-1.5 rounded-lg border px-3 py-1.5 text-xs font-semibold text-[#242423]/60 transition hover:border-[#242423]/25 hover:text-[#242423]"
        >
          <Plus size={12} /> Topik
        </button>
      </div>
      ...
      {modal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/20">
          <div className="mx-4 w-full max-w-sm overflow-hidden rounded-2xl bg-white shadow-xl">
            <div className="border-[#242423]/8 flex items-center justify-between border-b px-5 py-4">
              <span className="text-sm font-bold text-[#242423]">
                {modal.type === "pillar"
              ...
              <button
                ...
                className="text-[#242423]/40 hover:text-[#242423]"
              >
                <X size={14} />
              </button>
            </div>
            <div className="space-y-3 px-5 py-4">
              {modal.type === "pillar" && (
                <>
                  <input
                    ...
                    className={inputCls}
                    placeholder="Nama niche (e.g. UMKM Digital)"
                  />
                  <input
                    value={pName}
                    onChange={(e) => setPName(e.target.value)}
                    className={inputCls}
                    placeholder="Nama pillar (e.g. SEO untuk UMKM)"
                  />
                  <button
                    onClick={handleCreatePillar}
                    disabled={saving || !pNiche.trim() || !pName.trim()}
                    className="w-full rounded-lg bg-[#f5a700] py-2.5 text-sm font-bold text-white transition hover:bg-[#f5a700]/90 disabled:opacity-40"
                  >
              ...
              {modal.type === "assign" && (
                <>
                  <div className="mb-2 text-xs text-[#242423]/50">
                    Pilih artikel yang akan dihubungkan:
                  </div>
                  <div className="max-h-60 space-y-1 overflow-y-auto">
                    {unassignedArticles.length === 0 ? (
                      <div className="py-4 text-center text-xs text-[#242423]/40">
                        Semua artikel sudah terhubung ke pillar.
                      </div>
                    ) : (
                      unassignedArticles.map((a) => (
                        <label
                          key={a.id}
                          className="hover:bg-[#242423]/3 flex cursor-pointer items-start gap-2 rounded-lg p-2"
                        >
                          <input
                            type="checkbox"
                          ...
                          />
                          <div>
                            <div className="text-xs font-semibold leading-snug text-[#242423]">
                              {a.title}
                            </div>
                            <div className="text-[10px] text-[#242423]/40">
