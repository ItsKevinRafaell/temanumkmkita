...
(function () {
  ...
  // Resolve base demo folder from this page's own path, so partials always
  // load from the CURRENT demo (no hardcoded slug). e.g.
  // /portofolio/presisi-teknik/index.html -> /portofolio/presisi-teknik/partials/
  function demoBase() {
    const parts = window.location.pathname.split("/").filter(Boolean);
    const i = parts.indexOf("porto");
    if (i !== -1 && parts[i + 1]) return "/portofolio/" + parts[i + 1] + "/partials/";
    return "partials/";
  }
  const PARTIAL_BASE = demoBase();

  slots.forEach(function (slot) {
    const name = slot.getAttribute("data-include"); // "nav" / "footer"
    const url = PARTIAL_BASE + name + ".html";
